...
static void LCD_Disp_WrDatNib(uint8 nibble) ;
static void LCD_Disp_WrCntrlNib(uint8 nibble) ;
...
void LCD_Disp_PrintString(char8 const string[]) 
{
    ...
    {
        LCD_Disp_WriteData((uint8)current);
...
{
    LCD_Disp_WriteData((uint8)character);
}
...
void LCD_Disp_IsReady(void) 
{
    uint8 value;
    uint32 timeout;
    timeout = LCD_Disp_READY_DELAY;

    /* Clear LCD port*/
    LCD_Disp_PORT_DR_REG &= ((uint8)(~LCD_Disp_PORT_MASK));
    ...
    */
        ...
        value = LCD_Disp_PORT_PC_REG & ((uint32) (~ LCD_Disp_DM_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_Disp_PORT_PC_REG = value | LCD_Disp_HIGH_Z_DATA_DM;
        ...
        /* Mask off data pins to clear old values out */
        value = LCD_Disp_PORT_DM0_REG & ((uint8)(~LCD_Disp_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_Disp_PORT_DM0_REG = value | (LCD_Disp_HIGH_Z_DM0 & LCD_Disp_DATA_MASK);

        /* Mask off data pins to clear old values out */
        value = LCD_Disp_PORT_DM1_REG & ((uint8)(~LCD_Disp_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_Disp_PORT_DM1_REG = value;

        /* Mask off data pins to clear old values out */
        value = LCD_Disp_PORT_DM2_REG & ((uint8)(~LCD_Disp_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_Disp_PORT_DM2_REG = value;
    
    #endif /* CY_PSOC4 */
    
    /* Make sure RS is low */
    LCD_Disp_PORT_DR_REG &= ((uint8)(~LCD_Disp_RS));
    ...
    do
    {
        ...
        /* Set enable low */
        LCD_Disp_PORT_DR_REG &= ((uint8)(~LCD_Disp_E));
    ...
    LCD_Disp_PORT_DR_REG &= ((uint8)(~LCD_Disp_RW));
    ...
    LCD_Disp_PORT_DR_REG &= ((uint8)(~LCD_Disp_PORT_MASK));
        ...
        value = LCD_Disp_PORT_PC_REG & ((uint32)(~ LCD_Disp_DM_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_Disp_PORT_PC_REG = value | LCD_Disp_STRONG_DATA_DM;
        ...
        */
        LCD_Disp_PORT_DM0_REG &= ((uint8)(~LCD_Disp_DATA_MASK));
...
static void LCD_Disp_WrDatNib(uint8 nibble) 
{
    ...
    /* Clear data pins */
    LCD_Disp_PORT_DR_REG &= ((uint8)(~LCD_Disp_DATA_MASK));

    /* Write data, bring E high */
    #if(0u != LCD_Disp_PORT_SHIFT) /* MISRA forbids shift by 0 so need to handle that */
        LCD_Disp_PORT_DR_REG |= 
            (LCD_Disp_E | ((uint8)(((uint8) nibble) << LCD_Disp_PORT_SHIFT)));
    #else
        LCD_Disp_PORT_DR_REG |= (LCD_Disp_E | nibble);
    #endif /* (0u != LCD_Disp_PORT_SHIFT) */
...
static void LCD_Disp_WrCntrlNib(uint8 nibble) 
{
    ...
    LCD_Disp_PORT_DR_REG &= ((uint8)(~(LCD_Disp_RS | LCD_Disp_RW)));
    ...
    /*******************************************************************************
    *  Function Name: LCD_Disp_PrintInt8
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        static char8 const CYCODE LCD_Disp_hex[16u] = "0123456789ABCDEF";
    ...
    /*******************************************************************************
    *  Function Name: LCD_Disp_PrintInt16
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        LCD_Disp_PrintInt8((uint8)(value >> LCD_Disp_U16_UPPER_BYTE_SHIFT));
        LCD_Disp_PrintInt8((uint8)(value & LCD_Disp_U16_LOWER_BYTE_MASK));
    }

    
    /*******************************************************************************
    *  Function Name: LCD_Disp_PrintInt32
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        uint8 shift = LCD_Disp_32_BIT_SHIFT;
        ...
        while (shift != 0u)
        {
            ...
            */
            shift -= LCD_Disp_8_BIT_SHIFT;
            
            /* Print 8 bits of uint32 hex number */
            LCD_Disp_PrintInt8((uint8) ((uint32) (value >> shift)));
        }
    }
    
    
    /*******************************************************************************
    *  Function Name: LCD_Disp_PrintNumber
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    *  Function Name: LCD_Disp_PrintU32Number
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        uint8 tmpDigit;
        char8 number[LCD_Disp_NUMBER_OF_REMAINDERS_U32 + 1u];
        uint8 digIndex = LCD_Disp_NUMBER_OF_REMAINDERS_U32;
        ...
        */
        ...
        while(value >= LCD_Disp_TEN)
        {
            ...
            */
            tmpDigit = (uint8) (((uint8) (value % LCD_Disp_TEN)) + (uint8) LCD_Disp_ZERO_CHAR_ASCII);
        ...
        */
        ...
        /* Print out number */
        LCD_Disp_PrintString(&number[digIndex]);
    }
